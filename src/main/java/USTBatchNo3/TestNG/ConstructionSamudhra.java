...
//			List<Price> res=details.stream().filter(t->((t.getCity().toLowerCase()).equals(city))).toList();
		...
		public static int  construction(List<Price> details,int choice,String city,int sq) {
			List<Price> result=details.stream().filter(t->((t.getCity().toLowerCase()).equals(city))).toList();
